...
const C = {
  ...
  warmBg: "FFF8F0", lightTeal: "E0F5F5", darkMint: "01967A",
};
const FONT = "微软雅黑";
const TOTAL = 24;
...
async function main() {
  ...
  const iconDefs = [
    ...
    ["question", fa.FaQuestionCircle, C.orange], ["road", fa.FaRoad, C.deepBlue],
    ["flag", fa.FaFlag, C.red], ["trophy", fa.FaTrophy, C.orange],
    ...
    ["balanceW", fa.FaBalanceScale, "#FFFFFF"], ["lightbulbW", fa.FaLightbulb, "#FFFFFF"],
    ["questionW", fa.FaQuestionCircle, "#FFFFFF"], ["roadW", fa.FaRoad, "#FFFFFF"],
  ];
  ...
  const toc = [
    { num: "01", title: "桌面端的需求与挑战", desc: "运维痛点、技术选型与架构概览", icon: icons.desktop, color: C.deepBlue },
    { num: "02", title: "A2UI — 从需求中诞生的方案", desc: "技术架构、核心模块、渲染机制、应用场景", icon: icons.cubes, color: C.mint },
    { num: "03", title: "传统实现 vs A2UI 方案", desc: "对比分析与效率提升", icon: icons.balance, color: C.teal },
    { num: "04", title: "桌面端也是从 0 到 1", desc: "AI 辅助下从零开始完成开发", icon: icons.rocket, color: C.navy },
    { num: "05", title: "总结与展望", desc: "技术演进与未来思考", icon: icons.star, color: C.deepBlue },
  ];
  toc.forEach((t, i) => {
    const y = 1.1 + i * 0.82;
    addCard(s, 0.6, y, 8.8, 0.7, { accentLeft: t.color });
    addIcon(s, t.icon, 0.9, y + 0.15, 0.35);
    s.addText(t.num, { x: 1.45, y: y + 0.08, w: 0.5, h: 0.25, fontSize: 18, fontFace: FONT, color: t.color, bold: true, margin: 0 });
    s.addText(t.title, { x: 2.05, y: y + 0.08, w: 5, h: 0.25, fontSize: 15, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
    s.addText(t.desc, { x: 2.05, y: y + 0.38, w: 7, h: 0.22, fontSize: 10, fontFace: FONT, color: C.mutedText, margin: 0 });
  });
  addPageNum(s, 2);

  // ===== S3: Section 01 =====
  s = pres.addSlide(); s.background = { color: C.sectionBg };
  s.addShape(pres.shapes.RECTANGLE, { x: 0.6, y: 2.5, w: 0.15, h: 1.2, fill: { color: C.deepBlue } });
  s.addText("01", { x: 0.6, y: 1.5, w: 2, h: 0.7, fontSize: 48, fontFace: FONT, color: C.deepBlue, bold: true, margin: 0, transparency: 30 });
  s.addText("桌面端的需求与挑战", { x: 1.0, y: 2.5, w: 8, h: 0.7, fontSize: 36, fontFace: FONT, color: C.white, bold: true, margin: 0 });
  s.addText("运维痛点、技术选型与架构概览", { x: 1.0, y: 3.2, w: 8, h: 0.5, fontSize: 18, fontFace: FONT, color: "CADCFC", margin: 0 });
  addIcon(s, icons.desktopW, 8.5, 1.0, 1.0);

  // ===== S4: 运维痛点 =====
  s = pres.addSlide(); s.background = { color: C.lightBg }; addTopBar(s);
  s.addText("运维痛点：为什么需要海豚？", { x: 0.6, y: 0.3, w: 8, h: 0.6, fontSize: 28, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "01  桌面端的需求与挑战");
  const painPoints = [
    { title: "重复性事务", desc: "大量工单、权限申请等重复操作\n消耗运维人力，效率低下", icon: icons.cog, color: C.red },
    { title: "经验依赖", desc: "问题排查依赖个人经验\n知识难以沉淀和传承", icon: icons.brain, color: C.orange },
    { title: "信息孤岛", desc: "缺乏统一入口\n各系统割裂，信息分散", icon: icons.search, color: C.deepBlue },
    { title: "AI 难触达", desc: "AI 能力无法以交互式界面\n直接触达用户", icon: icons.robot, color: C.navy },
  ];
  painPoints.forEach((p, i) => {
    const xp = 0.6 + i * 2.25;
    addCard(s, xp, 1.1, 2.05, 2.4, { accentTop: p.color });
    addIcon(s, p.icon, xp + 0.15, 1.3, 0.35);
    s.addText(p.title, { x: xp + 0.6, y: 1.3, w: 1.3, h: 0.3, fontSize: 14, fontFace: FONT, color: p.color, bold: true, margin: 0 });
    s.addText(p.desc, { x: xp + 0.15, y: 1.75, w: 1.8, h: 1.0, fontSize: 10, fontFace: FONT, color: C.darkText, margin: 0 });
  });
  addCard(s, 0.6, 3.7, 8.8, 1.2, { accentLeft: C.mint });
  s.addText("海豚的定位", { x: 0.9, y: 3.8, w: 2, h: 0.3, fontSize: 14, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  const cv = [
    { label: "统一入口", desc: "一个应用覆盖运维全场景" },
    { label: "智能交互", desc: "自然语言驱动，降低门槛" },
    { label: "A2UI 赋能", desc: "AI 动态生成界面" },
    { label: "高效协作", desc: "会话全流程线上化" },
  ];
  cv.forEach((v, i) => {
    const xp = 0.9 + i * 2.2;
    s.addShape(pres.shapes.RECTANGLE, { x: xp, y: 4.2, w: 1.9, h: 0.5, fill: { color: C.lightMint } });
    s.addText(v.label, { x: xp + 0.1, y: 4.22, w: 1.7, h: 0.22, fontSize: 12, fontFace: FONT, color: C.teal, bold: true, margin: 0 });
    s.addText(v.desc, { x: xp + 0.1, y: 4.45, w: 1.7, h: 0.22, fontSize: 9, fontFace: FONT, color: C.mutedText, margin: 0 });
  });
  addPageNum(s, 4);

  // ===== S5: 技术选型与架构 =====
  s = pres.addSlide(); s.background = { color: C.lightBg }; addTopBar(s);
  s.addText("技术选型与架构概览", { x: 0.6, y: 0.3, w: 8, h: 0.6, fontSize: 28, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "01  桌面端的需求与挑战");
  addCard(s, 0.6, 1.1, 4.2, 2.2, { accentLeft: C.deepBlue });
  ...
  const ts = [
    ...
    { l: "数据流", v: "珊瑚系统 → JSON Schema" },
  ];
  ts.forEach((t, i) => {
    s.addText(t.l, { x: 0.9, y: 1.6 + i * 0.28, w: 1.2, h: 0.22, fontSize: 10, fontFace: FONT, color: C.mutedText, margin: 0 });
    s.addText(t.v, { x: 2.1, y: 1.6 + i * 0.28, w: 2.5, h: 0.22, fontSize: 11, fontFace: FONT, color: C.deepBlue, bold: true, margin: 0 });
  });
  addCard(s, 5.2, 1.1, 4.2, 2.2, { accentLeft: C.mint });
  ...
  layers.forEach((l, i) => {
    s.addShape(pres.shapes.RECTANGLE, { x: 5.5, y: 1.6 + i * 0.35, w: 0.06, h: 0.25, fill: { color: l.c } });
    s.addText(l.n, { x: 5.7, y: 1.58 + i * 0.35, w: 1.0, h: 0.25, fontSize: 11, fontFace: FONT, color: l.c, bold: true, margin: 0 });
    s.addText(l.d, { x: 6.7, y: 1.58 + i * 0.35, w: 2.5, h: 0.25, fontSize: 10, fontFace: FONT, color: C.mutedText, margin: 0 });
  });
  addCard(s, 0.6, 3.55, 8.8, 1.3, {});
  s.addText("核心功能", { x: 0.9, y: 3.65, w: 2, h: 0.25, fontSize: 13, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  const feats = [
  ...
  feats.forEach((f, i) => {
    const xp = 0.9 + i * 2.1;
    s.addShape(pres.shapes.RECTANGLE, { x: xp, y: 4.0, w: 1.9, h: 0.65, fill: { color: C.lightMint } });
    s.addText(f.title, { x: xp + 0.1, y: 4.02, w: 1.7, h: 0.28, fontSize: 11, fontFace: FONT, color: f.color, bold: true, margin: 0 });
    s.addText(f.desc, { x: xp + 0.1, y: 4.32, w: 1.7, h: 0.28, fontSize: 9, fontFace: FONT, color: C.mutedText, margin: 0 });
  });
  addPageNum(s, 5);

  // ===== S6: 需求引出问题（过渡页） =====
  s = pres.addSlide(); s.background = { color: C.warmBg }; addTopBar(s);
  s.addText("需求催生方案", { x: 0.6, y: 0.3, w: 8, h: 0.6, fontSize: 28, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "01 → 02  需求驱动方案");
  s.addText("面对这些痛点，我们提出了三个核心问题——", { x: 0.6, y: 1.1, w: 8, h: 0.4, fontSize: 14, fontFace: FONT, color: C.mutedText, margin: 0 });
  const transitions = [
    { question: "如何让 AI 输出可交互的界面？", answer: "A2UI 引擎：JSON Schema → UI", color: C.mint, icon: icons.puzzle },
    { question: "如何实现对话即界面？", answer: "珊瑚系统 + A2UI：动态渲染表单", color: C.teal, icon: icons.exchange },
    { question: "如何快速响应业务变化？", answer: "零代码扩展：Schema 更新即可", color: C.deepBlue, icon: icons.bolt },
  ];
  transitions.forEach((t, i) => {
    const yp = 1.7 + i * 1.15;
    addCard(s, 0.6, yp, 8.8, 0.95, { accentLeft: t.color });
    addIcon(s, t.icon, 0.9, yp + 0.2, 0.4);
    s.addText(t.question, { x: 1.5, y: yp + 0.08, w: 7.5, h: 0.35, fontSize: 16, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
    s.addText("→  " + t.answer, { x: 1.5, y: yp + 0.5, w: 7.5, h: 0.3, fontSize: 14, fontFace: FONT, color: t.color, bold: true, margin: 0 });
  });
  addPageNum(s, 6);
  ...
  s.addText("A2UI — 从需求中诞生的方案", { x: 1.0, y: 2.5, w: 8, h: 0.7, fontSize: 34, fontFace: FONT, color: C.white, bold: true, margin: 0 });
  ...
  archItems.forEach((a, i) => {
    const xp = 0.6 + i * 2.25;
    addCard(s, xp, 1.1, 2.05, 2.3, { accentTop: a.color });
  ...
  const flowSteps = ["珊瑚系统", "JSON Schema", "A2UIRoot", "Renderer", "Vue VNodes"];
  flowSteps.forEach((st, i) => {
    const xp = 1.9 + i * 1.5;
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: xp, y: 3.68, w: 1.2, h: 0.3, fill: { color: i === 0 ? C.orange : i === 4 ? C.navy : C.teal }, rectRadius: 0.05 });
  ...
  s.addText("A2UIRoot 核心枢纽：接收珊瑚系统 JSON → MessageProcessor 解析 → Renderer 映射 Vue 组件 → Context 共享状态 → Events 反馈用户操作", { x: 0.9, y: 4.65, w: 8.2, h: 0.4, fontSize: 10, fontFace: FONT, color: C.darkText, margin: 0 });
  addPageNum(s, 8);
  ...
  modules.forEach((m, i) => {
    ...
    const xp = 0.6 + col * 2.95, yp = 1.1 + row * 2.15;
  ...
  addCard(s, 0.6, 2.8, 4.8, 2.2, { accentLeft: C.codeBg });
  s.addShape(pres.shapes.RECTANGLE, { x: 0.66, y: 2.8, w: 4.68, h: 0.35, fill: { color: C.codeBg } });
  ...
  s.addText(codeText, { x: 0.8, y: 3.2, w: 4.3, h: 1.7, fontSize: 8, fontFace: "Consolas", color: C.codeText, margin: 0, valign: "top" });
  addCard(s, 5.8, 2.8, 3.6, 2.2, { accentLeft: C.mint });
  s.addText("Schema → UI 对应关系", { x: 6.1, y: 2.9, w: 3.0, h: 0.3, fontSize: 12, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  const mappings = [
    { schema: '"type": "Card"', ui: "卡片容器组件" },
    { schema: '"type": "TextField"', ui: "文本输入框" },
    { schema: '"type": "Button"', ui: "操作按钮" },
    { schema: '"props.title"', ui: "卡片标题栏" },
    { schema: '"props.field"', ui: "数据绑定字段" },
    { schema: '"props.action"', ui: "点击事件处理" },
  ];
  mappings.forEach((m, i) => {
    const yp = 3.3 + i * 0.3;
    s.addShape(pres.shapes.RECTANGLE, { x: 6.1, y: yp, w: 1.5, h: 0.22, fill: { color: C.lightBlue } });
    s.addText(m.schema, { x: 6.1, y: yp, w: 1.5, h: 0.22, fontSize: 8, fontFace: "Consolas", color: C.deepBlue, margin: 0, valign: "middle" });
    s.addText("→", { x: 7.65, y: yp, w: 0.25, h: 0.22, fontSize: 10, fontFace: FONT, color: C.mint, bold: true, margin: 0, align: "center", valign: "middle" });
    s.addText(m.ui, { x: 7.95, y: yp, w: 1.2, h: 0.22, fontSize: 9, fontFace: FONT, color: C.darkText, margin: 0, valign: "middle" });
  });
  addPageNum(s, 10);
  ...
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "02  A2UI 组件系统");
  addCard(s, 0.6, 1.1, 5.5, 3.8, {});
  s.addImage({ data: imgPlayground, x: 0.7, y: 1.2, w: 5.3, h: 3.6, sizing: { type: "contain", w: 5.3, h: 3.6 } });
  addCard(s, 6.4, 1.1, 3.0, 3.8, { accentTop: C.mint });
  s.addText("功能说明", { x: 6.7, y: 1.25, w: 2.4, h: 0.3, fontSize: 14, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  s.addText([
  ...
  ], { x: 6.7, y: 1.6, w: 2.5, h: 3.1, valign: "top", margin: 0 });
  addPageNum(s, 11);
  ...
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "02  A2UI 组件系统");
  addCard(s, 0.6, 1.1, 4.2, 4.0, { accentTop: C.mint });
  s.addText("工单提交", { x: 0.9, y: 1.2, w: 3.6, h: 0.35, fontSize: 16, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  s.addText("AI 识别工单类型后，珊瑚系统输出 JSON Schema，A2UI 实时渲染为工单填写表单，用户在对话中直接提交", { x: 0.9, y: 1.6, w: 3.6, h: 0.6, fontSize: 10, fontFace: FONT, color: C.darkText, margin: 0 });
  s.addImage({ data: imgOrder, x: 0.7, y: 2.3, w: 4.0, h: 2.6, sizing: { type: "contain", w: 4.0, h: 2.6 } });
  addCard(s, 5.2, 1.1, 4.2, 4.0, { accentTop: C.teal });
  ...
  s.addText("对比分析与效率提升", { x: 1.0, y: 3.2, w: 8, h: 0.5, fontSize: 18, fontFace: FONT, color: "CADCFC", margin: 0 });
  ...
  const tradCost = [
    { l: "技术栈", v: "Vue + Element Plus + API" }, { l: "开发周期", v: "2-4 周/功能" },
    { l: "维护成本", v: "高（硬编码表单）" }, { l: "技术难点", v: "动态表单、状态管理" },
  ];
  ...
  pains.forEach((p, i) => {
    const xp = 0.9 + i * 2.1;
    s.addText(p.label, { x: xp, y: 4.15, w: 1.9, h: 0.22, fontSize: 11, fontFace: FONT, color: C.red, bold: true, margin: 0 });
    s.addText(p.desc, { x: xp, y: 4.4, w: 1.9, h: 0.35, fontSize: 9, fontFace: FONT, color: C.mutedText, margin: 0 });
  });
  addPageNum(s, 14);
  ...
  // ===== S16: 以工单为例的对比 =====
  s = pres.addSlide(); s.background = { color: C.lightBg }; addTopBar(s);
  s.addText("以「工单提交」为例：传统 vs A2UI", { x: 0.6, y: 0.3, w: 8, h: 0.6, fontSize: 24, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "03  传统实现 vs A2UI 方案");
  addCard(s, 0.6, 1.1, 4.2, 3.8, { accentTop: C.red });
  s.addText("传统实现", { x: 0.9, y: 1.2, w: 3.6, h: 0.3, fontSize: 16, fontFace: FONT, color: C.red, bold: true, margin: 0 });
  s.addText([
    { text: "1. 产品定义工单字段与流程", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 4 } },
    { text: "2. 设计出交互稿、视觉稿", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 4 } },
    { text: "3. 前端编写 Vue 模板 + 表单验证", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 4 } },
    { text: "4. 后端开发工单 API + 数据库", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 4 } },
    { text: "5. 联调 → 测试 → 修复 → 上线", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 6 } },
    { text: "总耗时：2-4 周", options: { fontSize: 12, fontFace: FONT, color: C.red, bold: true } },
  ], { x: 0.9, y: 1.6, w: 3.6, h: 2.8, valign: "top", margin: 0 });
  addCard(s, 5.2, 1.1, 4.2, 3.8, { accentTop: C.mint });
  s.addText("A2UI 方案", { x: 5.4, y: 1.2, w: 3.8, h: 0.3, fontSize: 16, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  s.addText([
    { text: "1. AI 识别用户工单需求", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 4 } },
    { text: "2. 珊瑚系统输出工单 JSON Schema", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 4 } },
    { text: "3. A2UI 引擎实时渲染工单表单", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 4 } },
    { text: "4. 用户在对话中直接填写提交", options: { breakLine: true, fontSize: 10, fontFace: FONT, color: C.darkText, paraSpaceAfter: 6 } },
    { text: "总耗时：1-3 天", options: { fontSize: 12, fontFace: FONT, color: C.mint, bold: true } },
  ], { x: 5.4, y: 1.6, w: 3.8, h: 2.8, valign: "top", margin: 0 });
  s.addShape(pres.shapes.RECTANGLE, { x: 4.95, y: 1.3, w: 0.04, h: 3.4, fill: { color: C.mint, transparency: 50 } });
  addPageNum(s, 16);
  ...
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "04  桌面端也是从 0 到 1");
  ...
  // ===== S20: 开发周期与成果 =====
  s = pres.addSlide(); s.background = { color: C.lightBg }; addTopBar(s);
  s.addText("开发周期与 A2UI 成果展示", { x: 0.6, y: 0.3, w: 8, h: 0.6, fontSize: 26, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "04  桌面端也是从 0 到 1");
  addCard(s, 0.6, 1.1, 4.4, 1.8, { accentTop: C.deepBlue });
  s.addText("开发周期", { x: 0.9, y: 1.2, w: 3, h: 0.25, fontSize: 13, fontFace: FONT, color: C.deepBlue, bold: true, margin: 0 });
  const timeline = [
    { phase: "1-2周", task: "架构+A2UI集成", color: C.mint },
    { phase: "3-4周", task: "核心功能开发", color: C.teal },
    { phase: "5-6周", task: "桌面端适配", color: C.deepBlue },
    { phase: "7-8周", task: "测试+上线", color: C.navy },
  ];
  timeline.forEach((t, i) => {
    const xp = 0.9 + i * 1.0;
    s.addShape(pres.shapes.RECTANGLE, { x: xp, y: 1.55, w: 0.9, h: 0.7, fill: { color: t.color, transparency: 90 } });
    s.addShape(pres.shapes.RECTANGLE, { x: xp, y: 1.55, w: 0.04, h: 0.7, fill: { color: t.color } });
    s.addText(t.phase, { x: xp + 0.1, y: 1.57, w: 0.75, h: 0.22, fontSize: 9, fontFace: FONT, color: t.color, bold: true, margin: 0 });
    s.addText(t.task, { x: xp + 0.1, y: 1.8, w: 0.75, h: 0.4, fontSize: 8, fontFace: FONT, color: C.darkText, margin: 0 });
  });
  const resStats = [
    { num: "1人", label: "AI辅助开发" },
    { num: "8周", label: "从零到交付" },
  ...
  resStats.forEach((r, i) => {
    const xp = 0.9 + i * 1.0;
    s.addText(r.num, { x: xp, y: 2.4, w: 0.9, h: 0.22, fontSize: 12, fontFace: FONT, color: C.mint, bold: true, margin: 0, align: "center" });
    s.addText(r.label, { x: xp, y: 2.6, w: 0.9, h: 0.15, fontSize: 7, fontFace: FONT, color: C.mutedText, margin: 0, align: "center" });
  });
  addCard(s, 5.2, 1.1, 4.2, 1.8, { accentTop: C.mint });
  s.addText("A2UI 核心成果", { x: 5.5, y: 1.2, w: 3.6, h: 0.25, fontSize: 13, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  const achievements = [
    "桌面端 + Web 端双平台 A2UI 集成",
    "工单提交、网络权限申请场景落地",
    "珊瑚系统 JSON → A2UI 渲染",
    "从零开始，AI 辅助 8 周完成交付",
  ];
  achievements.forEach((a, i) => {
    s.addShape(pres.shapes.RECTANGLE, { x: 5.5, y: 1.55 + i * 0.22, w: 0.12, h: 0.12, fill: { color: C.mint } });
    s.addText(a, { x: 5.75, y: 1.5 + i * 0.22, w: 3.4, h: 0.22, fontSize: 9, fontFace: FONT, color: C.darkText, margin: 0 });
  });
  addCard(s, 0.6, 3.1, 4.2, 2.0, { accentTop: C.mint });
  s.addText("海豚桌面端", { x: 0.9, y: 3.2, w: 3.6, h: 0.25, fontSize: 12, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  s.addImage({ data: imgDesktop, x: 0.7, y: 3.5, w: 4.0, h: 1.45, sizing: { type: "contain", w: 4.0, h: 1.45 } });
  addCard(s, 5.2, 3.1, 4.2, 2.0, { accentTop: C.teal });
  s.addText("辛顿 · 海豚 Web 端", { x: 5.5, y: 3.2, w: 3.6, h: 0.25, fontSize: 12, fontFace: FONT, color: C.teal, bold: true, margin: 0 });
  s.addImage({ data: imgHinton1, x: 5.3, y: 3.5, w: 2.0, h: 1.45, sizing: { type: "contain", w: 2.0, h: 1.45 } });
  s.addImage({ data: imgHinton2, x: 7.4, y: 3.5, w: 2.0, h: 1.45, sizing: { type: "contain", w: 2.0, h: 1.45 } });
  addPageNum(s, 20);

  // ===== S21: AI 辅助开发心得 =====
  s = pres.addSlide(); s.background = { color: C.lightBg }; addTopBar(s);
  s.addText("AI 辅助开发的核心心得", { x: 0.6, y: 0.3, w: 8, h: 0.6, fontSize: 28, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "04  桌面端也是从 0 到 1");
  const insights = [
    { num: "01", title: "提问比编码更重要", desc: "精准描述需求，AI 才能给出高质量方案\n学会将模糊需求拆解为具体问题", color: C.mint, icon: icons.lightbulb },
    { num: "02", title: "渐进式验证", desc: "不要一次性生成大量代码\n每一步验证，快速迭代，降低返工成本", color: C.teal, icon: icons.check },
    { num: "03", title: "理解原理而非复制代码", desc: "AI 生成的代码需要理解其原理\n知其然更要知其所以然，才能排查问题", color: C.deepBlue, icon: icons.brain },
    { num: "04", title: "架构思维不可替代", desc: "AI 可以写代码，但架构设计需要人\n技术选型、模块划分、边界定义仍需人主导", color: C.navy, icon: icons.sitemap },
  ];
  insights.forEach((ins, i) => {
    const yp = 1.1 + i * 1.0;
    addCard(s, 0.6, yp, 8.8, 0.85, { accentLeft: ins.color });
    addIcon(s, ins.icon, 0.9, yp + 0.2, 0.35);
    s.addText(ins.title, { x: 1.45, y: yp + 0.05, w: 7.5, h: 0.3, fontSize: 14, fontFace: FONT, color: ins.color, bold: true, margin: 0 });
    s.addText(ins.desc, { x: 1.45, y: yp + 0.4, w: 7.5, h: 0.38, fontSize: 10, fontFace: FONT, color: C.mutedText, margin: 0 });
  });
  addPageNum(s, 21);
  ...
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "05  总结与展望");
  const outlook = [
  ...
  await pres.writeFile({ fileName: "d:\\work\\program\\tineco\\desk\\opsmind\\海豚-技术分享-v5.pptx" });
  console.log("PPT v5 generated successfully!");
}
