...
async function main() {
  ...
  // ===== S4: 运维痛点与海豚定位 =====
  s = pres.addSlide(); s.background = { color: C.lightBg }; addTopBar(s);
  s.addText("运维痛点与海豚定位", { x: 0.6, y: 0.3, w: 8, h: 0.6, fontSize: 28, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "01  桌面端的需求与挑战");
  addCard(s, 0.6, 1.1, 4.2, 2.2, { accentTop: C.red });
  s.addText("运维痛点", { x: 0.9, y: 1.2, w: 3.6, h: 0.35, fontSize: 16, fontFace: FONT, color: C.red, bold: true, margin: 0 });
  s.addText([
    ...
    { text: "缺乏统一入口，信息孤岛严重", options: { breakLine: true, fontSize: 11, fontFace: FONT, color: C.darkText, bullet: true, paraSpaceAfter: 4 } },
    { text: "AI 能力无法以交互式界面触达用户", options: { fontSize: 11, fontFace: FONT, color: C.darkText, bullet: true } },
  ], { x: 0.9, y: 1.65, w: 3.7, h: 1.5, valign: "top", margin: 0 });
  addCard(s, 5.2, 1.1, 4.2, 2.2, { accentTop: C.mint });
  s.addText("海豚定位", { x: 5.5, y: 1.2, w: 3.6, h: 0.35, fontSize: 16, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  s.addText([
    ...
    { text: "A2UI 引擎实现对话即界面", options: { breakLine: true, fontSize: 11, fontFace: FONT, color: C.darkText, bullet: true, paraSpaceAfter: 4 } },
    { text: "珊瑚系统 + A2UI 动态生成交互表单", options: { fontSize: 11, fontFace: FONT, color: C.darkText, bullet: true } },
  ], { x: 5.5, y: 1.65, w: 3.7, h: 1.5, valign: "top", margin: 0 });
  addCard(s, 0.6, 3.55, 8.8, 1.3, { accentTop: C.teal });
  s.addText("核心价值", { x: 0.9, y: 3.65, w: 2, h: 0.3, fontSize: 14, fontFace: FONT, color: C.teal, bold: true, margin: 0 });
  const cv = [
  ...
  cv.forEach((v, i) => {
    const xp = 0.9 + i * 2.2;
    s.addShape(pres.shapes.RECTANGLE, { x: xp, y: 4.05, w: 1.9, h: 0.6, fill: { color: C.lightMint } });
    s.addText(v.label, { x: xp + 0.1, y: 4.07, w: 1.7, h: 0.25, fontSize: 12, fontFace: FONT, color: C.teal, bold: true, margin: 0 });
    s.addText(v.desc, { x: xp + 0.1, y: 4.32, w: 1.7, h: 0.25, fontSize: 9, fontFace: FONT, color: C.mutedText, margin: 0 });
  });
  addPageNum(s, 4);
  ...
  // ===== S6: 从需求到方案（过渡页） =====
  s = pres.addSlide(); s.background = { color: C.lightBg }; addTopBar(s);
  s.addText("从需求到解决方案", { x: 0.6, y: 0.3, w: 8, h: 0.6, fontSize: 28, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
  ...
  transitions.forEach((t, i) => {
    const yp = 1.2 + i * 1.3;
    addCard(s, 0.6, yp, 8.8, 1.1, { accentLeft: t.color });
    addIcon(s, t.icon, 0.9, yp + 0.2, 0.4);
    s.addText(t.question, { x: 1.5, y: yp + 0.1, w: 7.5, h: 0.35, fontSize: 16, fontFace: FONT, color: C.darkText, bold: true, margin: 0 });
    s.addText("→  " + t.answer, { x: 1.5, y: yp + 0.55, w: 7.5, h: 0.35, fontSize: 14, fontFace: FONT, color: t.color, bold: true, margin: 0 });
  });
  addPageNum(s, 6);
  ...
  addAccent(s, 0.6, 0.85, 0.8); addSection(s, "04  桌面端也是从 0 到 1");
  addCard(s, 0.6, 1.1, 4.4, 1.5, { accentTop: C.deepBlue });
  ...
  timeline.forEach((t, i) => {
    const xp = 0.9 + i * 1.0;
    s.addShape(pres.shapes.RECTANGLE, { x: xp, y: 1.55, w: 0.9, h: 0.8, fill: { color: t.color, transparency: 90 } });
    s.addShape(pres.shapes.RECTANGLE, { x: xp, y: 1.55, w: 0.04, h: 0.8, fill: { color: t.color } });
    s.addText(t.phase, { x: xp + 0.1, y: 1.57, w: 0.75, h: 0.22, fontSize: 9, fontFace: FONT, color: t.color, bold: true, margin: 0 });
    s.addText(t.task, { x: xp + 0.1, y: 1.82, w: 0.75, h: 0.45, fontSize: 8, fontFace: FONT, color: C.darkText, margin: 0 });
  });
  // Resource stats
  const resStats = [
    { num: "1人", label: "AI辅助开发" },
  ...
  resStats.forEach((r, i) => {
    const xp = 0.9 + i * 1.0;
    s.addText(r.num, { x: xp, y: 2.45, w: 0.9, h: 0.22, fontSize: 12, fontFace: FONT, color: C.mint, bold: true, margin: 0, align: "center" });
    s.addText(r.label, { x: xp, y: 2.65, w: 0.9, h: 0.15, fontSize: 7, fontFace: FONT, color: C.mutedText, margin: 0, align: "center" });
  });
  // A2UI achievements
  addCard(s, 5.2, 1.1, 4.2, 1.5, { accentTop: C.mint });
  ...
  // Screenshots
  addCard(s, 0.5, 2.85, 4.4, 2.2, { accentTop: C.mint });
  s.addText("海豚桌面端", { x: 0.8, y: 2.95, w: 3.8, h: 0.25, fontSize: 12, fontFace: FONT, color: C.mint, bold: true, margin: 0 });
  s.addImage({ data: imgDesktop, x: 0.7, y: 3.25, w: 4.0, h: 1.65, sizing: { type: "contain", w: 4.0, h: 1.65 } });
  addCard(s, 5.1, 2.85, 4.4, 2.2, { accentTop: C.teal });
  s.addText("辛顿 · 海豚 Web 端", { x: 5.4, y: 2.95, w: 3.8, h: 0.25, fontSize: 12, fontFace: FONT, color: C.teal, bold: true, margin: 0 });
  s.addImage({ data: imgHinton1, x: 5.3, y: 3.25, w: 2.0, h: 1.65, sizing: { type: "contain", w: 2.0, h: 1.65 } });
  s.addImage({ data: imgHinton2, x: 7.4, y: 3.25, w: 2.0, h: 1.65, sizing: { type: "contain", w: 2.0, h: 1.65 } });
  addPageNum(s, 19);
  ...
  await pres.writeFile({ fileName: "d:\\work\\program\\tineco\\desk\\opsmind\\海豚-技术分享-v4.pptx" });
